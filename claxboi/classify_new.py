...
if __name__ == "__main__":
    ...
    if filename.split(".")[-1][:3]=="fit":
        ...
        # Sanitize column names: replace hyphens with underscores
        # (e.g. BP-RP -> BP_RP) because np.genfromtxt cannot handle hyphens in names.
        # Skip if the target name already exists (e.g. BP_RP computed by om_compute_features).
        for col in list(t.colnames):
            if '-' in col:
                new_name = col.replace('-', '_')
                if new_name in t.colnames:
                    t.remove_column(col)  # drop the hyphenated duplicate
                else:
                    t.rename_column(col, new_name)
    ...
    print('This program will classify UV/optical sources from %s using %d of their properties:\n'%(filename, nprop)+',\n'.join([', '.join(properties[i:i+7]) for i in range(0, nprop, 7)])+'\n')
    ...
    def proba_fast(isrc, global_weights):
        ...
        for cat in range(ncat): #location, spectrum, multiwv, variabty
            try:
                pbCat = np.array([(np.nanprod(pbgood[ic][icat[cat][~SourcesNan[isrc]]])*np.prod(pbnull[ic][icat[cat][SourcesNan[isrc]]])**alpha)**(5*global_weights[cat]/(2*len(pbgood[0])+alpha*len(pbnull[0]))) for ic in range(ncla)])
        ...
        for cat in range(ncat):
            ...
            gw2.pop(cat)
            pbCl2 = np.prod(np.array(pbCat2), axis=0)**(8/sum(gw2))
        ...
        if isrc in totest:#classes[isrc] == 2 and isrc%200 == np.random.randint(200):# isrc%3000 == 13:
            ...
            for cat in range(ncat):
                ...
                string += ' %s'%(np.prod(pbgood[ic][icat[cat][~SourcesNan[isrc]]])/sum([np.prod(pbgood[iC][icat[cat][~SourcesNan[isrc]]]) for iC in range(ncla)]))
                if np.isnan(np.prod(pbgood[ic][icat[cat][~SourcesNan[isrc]]])/sum([np.prod(pbgood[iC][icat[cat][~SourcesNan[isrc]]]) for iC in range(ncla)])):
                    try:
                ...
                string += ' %s\n'%(np.prod(pbnull[ic][icat[cat][SourcesNan[isrc]]])/sum([np.prod(pbnull[iC][icat[cat][SourcesNan[isrc]]]) for iC in range(ncla)]))
            pbCl2 = np.ones(ncla)
            for cat in range(ncat):
    ...
    def f1score(global_weights, outfile=fout):
        ...
        if C==-1:
            ...
            avg = np.mean(f1_per_class)
            p0C1, p1C1, p1C0 = -1, -1, -1
        else:
        ...
        print(' '.join(['%.3f'%gw for gw in global_weights]), p0C1, p1C1, p1C0, avg, file=outfile)
    ...
    if optimize_coeffs:
        ...
        try:
            res = differential_evolution(f1score, [(0,1)]*(len(global_coeffs)-len(categories))+[(0,10)]*len(categories), disp=1)
            print(res.x, res.fun, res.message, res.nit)
            global_coeffs = res.x
        except (KeyboardInterrupt, Exception) as e:
            print(f"Optimization interrupted or failed: {e}")
            fout.close()
            outfile2 = filename.rsplit('.', 1)[0] + '_optimsteps.dat'
            best_coeffs = None
            if os.path.isfile(outfile2) and os.path.getsize(outfile2) > 0:
            ...
            if best_coeffs is not None:
                global_coeffs = best_coeffs
            else:
                print("No valid optimization steps found. Keeping current global_coeffs.")

        str_coeffs = str(list(np.round(global_coeffs, 2)))
        # Update configfile using Python instead of shell sed commands
        with open(configfile, 'r') as cf:
            config_text = cf.read()
        import re
        config_text = re.sub(r'global_coeffs:.*', 'global_coeffs: %s' % str_coeffs, config_text)
