...
def make(SourcesCsv, properties, Classes=[0,1,2], equipart=False, fraction=1, plotdistrib=False,N=100,verbose=False,dirout='classif/distribtest/', custom_pty=[],dumb=False, scale=None):
    ...
    for ip in range(len(properties)):
        ...
        for ic in range(len(Classes)):
            ...
            if np.sum(ind) == 0:
                # No training data for this class/property — use flat distribution
                print("WARNING: property %s has no values for class %s — using flat prior"%(p,Classes[ic]))
                dsm_kde = np.ones_like(xfinal)
                dl.append(dsm_kde)
                continue
            ...
            try:
                ...
                print("WARNING: property %s KDE fit failed for class %s — using flat prior"%(p,Classes[ic]))
                dsm_kde = np.ones_like(xfinal)
                dl.append(dsm_kde)
                continue
